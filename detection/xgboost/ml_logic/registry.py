...
def save_model(model: keras.Model = None) -> None:
    ...
    # Save model locally
    model_path = os.path.join(LOCAL_REGISTRY_PATH, "models/xgboost", f"{timestamp}.h5")
    ...
    if MODEL_TARGET == "gcs":
        ...
        blob = bucket.blob(f"training/xgboost/models/{model_filename}")
...
def load_model(stage="Production") -> keras.Model:
    ...
    if MODEL_TARGET == "local":
        ...
        client = storage.Client()
        blobs = list(client.get_bucket(BUCKET_NAME).list_blobs(prefix="model/xgboost"))
